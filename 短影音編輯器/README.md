...
- `layout.ratio`：`"9:16"`（預設，直式，畫布 1080x1920）或 `"16:9"`
  （橫式，畫布 1920x1080），`render.py` 會依這個值切換畫布尺寸跟三區版面
  的留白（`TITLE_ZONE`/`CTA_ZONE`/`SUBTITLE_ZONE`，見 `render.py` 裡對應
  段落的常數與註解）。橫式因為畫布高度只有直式的一半多一點，字幕跟 CTA
  卡之間留的空間比例上緊很多，這幾個常數是照真實素材輸出、截圖檢查過
  才調出來的，不是憑感覺猜的數字——之後如果要再調整這幾個常數，務必
  重新輸出一小段、截圖確認標題／主影片／字幕／CTA 沒有互相蓋到，不要
  只靠算式推論就當作沒問題（2026-09 曾經第一版算式讓橫式主影片直接蓋滿
  整個畫布，把標題卡跟 CTA 卡全部蓋掉；修完第一版又忘了留字幕的空間，
  字幕疊在 CTA 卡文字上；修完留白之後，主影片的 `scale` 濾鏡沒有搭配
  `force_original_aspect_ratio=increase`＋`crop`，`video_h` 縮小後跟
  來源影片的長寬比不再一樣，畫面被直接硬拉伸變形——三次都是實際輸出
  截圖才抓到，光看程式碼推論看不出來）。主影片跟 B-roll 疊圖現在都是
  同一招：先等比例縮放到蓋滿目標框，再裁掉多出來的部分，不要直接
  `scale=W:H` 硬套，那樣只要目標框比例跟來源影片不一樣就會變形。
...
- **用 Git Bash 呼叫 `new_project.py`/`render.py`/`verify_render.py`，
  路徑參數一律優先用 Windows 風格（`C:\Users\...`），不要用 POSIX
  風格（`/c/Users/...`）**：Git Bash（MSYS）平常會自動把 POSIX 路徑轉
  成 Windows 路徑再傳給非 MSYS 執行檔（像 `python.exe`），但遇到含
  空白／中文／方括號的長路徑常常轉換失敗，導致 `/c/Users/x` 原封不動
  傳進 Python，而 Python 在 Windows 上會把開頭的 `/` 當成「目前所在
  磁碟的根目錄」，解析成 `C:\c\Users\x`（多套一層假的 `c` 資料夾），
  變成「找不到檔案」——實測踩過。三支腳本的 CLI 入口已經改用
  `av_tools.resolve_cli_path()`，會自動偵測這個特徵並試著修正、印出
  `[路徑修正]` 訊息，但這只是安全網，**優先做法還是直接用 PowerShell
  執行這三支腳本，或在 Bash 裡就給 Windows 風格路徑**，不要靠安全網
  當常態。
- **這台工具鏈同時被 PowerShell 和 Git Bash 呼叫，兩者對「終端機期待
  的輸出編碼」認知不一樣**：早期版本 `av_tools.log()` 假設 Windows
  主控台是 cp950（繁中 Big5）codepage，PowerShell 底下大致還看得懂，
  但同樣的輸出丟進 Git Bash（本質上假設 UTF-8）看到的就是整串亂碼
  ——實測踩過，連腳本自己 `print()` 出來的路徑錯誤訊息都亂碼到看不出
  內容。`av_tools.py` 現在在 import 時就把 `sys.stdout`/`sys.stderr`
  統一 `reconfigure(encoding="utf-8", errors="replace")`，三支主要
  腳本（`new_project.py`／`render.py`／`verify_render.py`）都會 import
  `av_tools`，所以這個修正是全域生效的，不用每支腳本自己再處理一次。
  如果之後要寫新的臨時除錯腳本（例如單獨重新轉錄一小段音檔核對內容），
  一樣要在檔案最上面加同樣的 `reconfigure`，或者乾脆 `import av_tools`
  借用它 import 時就做好的全域設定，不要假設 Windows 主控台的預設
  codepage 印得出中文——這在 Git Bash 底下幾乎必炸。
